...
function handleButtonClick(event) {
    ...
    var delayMilliseconds = document.getElementById("time").value * 60000; // Convert minutes to milliseconds

    // Check if the button is already disabled
    if (button.hasAttribute("disabled")) {
        // The button is disabled, it means its in the delayed state
        event.preventDefault(); // Prevent the form from submitting immediately
        return;
    }

    // Button is not disabled, proceed with immediate submission
    document.getElementById("myForm").submit(); // Manually submit the form

// Function to introduce a delay when the page loads
document.addEventListener("DOMContentLoaded", function() {
    var delayMilliseconds = document.getElementById("time").value * 60000; // Convert minutes to milliseconds

    // Introduce a delay based on the input value
    introduceDelay(delayMilliseconds);
});

// Function to introduce a delay before submission
function introduceDelay(delayMilliseconds) {
    var button = document.getElementById("submitButton");

    // Disable the button during the delay
    button.setAttribute("disabled", "true");

    // Set a timeout to enable the button after the delay
    setTimeout(function() {
